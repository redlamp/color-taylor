...
`hue-at:<deg>` (a degree on the Color Editor's hue strip as a place rather than a
control — `hue-at:120` is where 120 sits on `#hue-bar` — with a ring radius wide
enough to stand clear of a strip narrower than any ring that would read as one;
beat 3 rings the primaries and the combinations there at the same moment it rings
them on the hexagon, so the angle is shown as a direction and as a position at
once), `editor-hue-handle` (the marker on that strip, `#hue-bar-arrow`, where
`hex-hue-label` is the hexagon's own),
...
saved — so there the box is the whole grid.
