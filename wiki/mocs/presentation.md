...
- [[plan-walkthrough-entry]] - how it ships: entry from the About panel, `?present=` paused, the 800px gate, the play() gesture, and what was built.
- [[plan-camera-action]] - deferred: a `camera` cue that zooms and slides inside the app, parked on `feature/camera-action`.
