...
- **Impact highlights.** While the pointer holds a control, everything else that moved lights until release. `useImpact` diffs the readouts against a snapshot taken at the press, keyed by `data-hold` tags; the keyline tokens are in `src/utils/highlight.ts` and its two colours in `index.css`. The ground for the self-running demo — [[plan-picker-demo]].
