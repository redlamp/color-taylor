...
- **Lab panel timings** — `?perf` on a lab URL (`localhost:5173/lab/cie.html?perf`). Prints one console line per frame that did work, naming which figure did it: `[perf] 18.4ms frame · solid 12.1 · morph mesh 4.6 (1)`. The leading figure is wall-clock since the previous line, so it is the frame interval rather than a sum of the parts; a bracketed count means that bucket ran more than once in the frame. Off by default, like `?fps`, and it measures **JavaScript** — WebGL calls return once the work is queued, so a slow GPU shows up in the frame interval rather than in the `solid` bucket. Same caveat as the fps meter: read it in a real browser, not the preview pane.
- The labs deploy with the app: each `lab/<name>.html` builds to `<base>lab/<name>.html`, with an index at `<base>lab/`.
- **All four CIE lab panels read the live colour.** Panels 3 and 4 were once fed a `useDeferredValue` copy, from when a colour change redrew the whole solid (2.2 s a pointer move). That cost is gone, and under a real mouse the deferral starved: every move is urgent, so the deferred render only ran in gaps and the figures froze mid-drag. Now each figure is `memo`'d, takes the colour through a memoised key, and coalesces its own draw to one per animation frame; a production build holds handle and solid in step at the display's refresh rate. If a figure gets expensive again, throttle *its draw* (30 fps is acceptable) rather than deferring its props.
