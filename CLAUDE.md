...
- **Panels 2, 3 and 4 of the CIE lab are deliberately behind panel 1.** The hexagon is a control and the rest are readouts of it, so `CieLab` feeds the morph and the solid a `useDeferredValue` copy of the colour and both coalesce their draw to one per animation frame. Sharing a deadline cost a pointer move 2.2 s with the solid mounted, against 48 ms without it. If you add a figure to that page, put it on the deferred copy unless it is as cheap as the chromaticity diagram, which is live because watching its dot *not* move is the page's whole argument.
