...
- `?fps` on any URL — `localhost:5173/?fps#/` — shows a frame-rate meter (fps, p95 and worst frame time, hitch count over ~2 s). A query param rather than a setting so it needs no `localStorage` owner; works on the deployed site. It writes to the DOM through a ref and never re-renders the app, so it does not measure itself. The Browser preview pane barely fires `requestAnimationFrame`, so read it in a real browser.
