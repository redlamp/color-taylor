/**
 * CIE 1931 2-degree standard observer colour-matching functions, 360-830 nm at
 * 5 nm intervals. Each row is [wavelength nm, x-bar, y-bar, z-bar].
 *
 * This is measured data, not a curve anyone here fitted. It is what makes the
 * spectral locus - the horseshoe - the real locus of monochromatic light
 * rather than a drawing of one, and the diagram exists to show exactly that
 * shape, so nothing about it may be approximated. The digits are CVRL's own,
 * with only the trailing zeros their CSV pads to a fixed width removed; the
 * generator refused to write a row whose trimmed value did not parse back to
 * the source value.
 *
 * Source: Colour & Vision Research Laboratory (CVRL), UCL Institute of
 * Ophthalmology - the CIE's tabulation, redistributed by CVRL.
 *   http://cvrl.ioo.ucl.ac.uk/database/data/cmfs/ciexyz31.csv
 *   (mirrored at http://www.cvrl.org/ - "CIE 1931 2-deg, XYZ CMFs")
 * Downloaded 2026-09-20.
 *
 * Checks run against it, and kept in cie.test.ts:
 * - y-bar peaks at 555 nm with the value 1.000000 exactly, as the standard
 *   defines it, and x-bar(600) = 1.0622, y-bar(600) = 0.6310 - the published
 *   table's numbers.
 * - Every sRGB primary, every Display P3 primary and the D65 white point fall
 *   inside the locus these rows generate. A primary outside it would mean the
 *   data or the transform was wrong, and nothing downstream would be worth
 *   trusting.
 * - The locus is convex to within 7.7e-5 in xy and never reverses direction as
 *   wavelength rises.
 * - Above 700 nm the chromaticity stops moving: every row from 700 to 830 nm
 *   lands within 1.5e-7 of the 700 nm point. That is why the drawn horseshoe
 *   stops at 700: the remaining 26 samples would be drawn on top of it.
 */
export const CIE_1931_2DEG: ReadonlyArray<readonly [number, number, number, number]> = [
