...
export function fieldPointFor(r: number, g: number, b: number, target: MorphTarget): FieldPoint {
  const m = morphPoint(r, g, b, target);
 ...
 */
export function buildMorphMesh(brightness: number, target: MorphTarget): MorphMesh {
  const verts = (NR + 1) * NA;
  ...
  for (let j = 0; j <= NR; j++) {
    ...
    for (let i = 0; i < NA; i++) {
      ...
      const m = morphPoint(c.r, c.g, c.b, target);
      const v = j * NA + i;
...
out vec3 vColor;
void main() {
  vec2 p = mix(aPos0, aPos1, uT) / uSize * 2.0 - 1.0;
...
export interface HexMorphRenderer {
  ...
  /** One frame at morph position `t`, 0 to 1. */
  draw(t: number): void;
  destroy(): void;
}
